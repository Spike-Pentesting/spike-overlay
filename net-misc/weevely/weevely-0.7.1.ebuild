...
# Portage.
SRC_URI="https://github.com/downloads/epinna/Weevely/${P}.tar.gz"
...
src_install() {
	...
	dodir /opt/spike
	dodir /opt/spike/weevely
	WEEVELY="/opt/spike/weevely"
	insinto "${WEEVELY}"
	doins "${PN}".py
	dodir "${WEEVELY}"/core
	dodir "${WEEVELY}"/core/http
	dodir "${WEEVELY}"/modules
	dodir "${WEEVELY}"/modules/file 
	dodir "${WEEVELY}"/modules/system  
	dodir "${WEEVELY}"/modules/find 
	dodir "${WEEVELY}"/modules/shell  
	dodir "${WEEVELY}"/modules/sql  
        dodir "${WEEVELY}"/modules/net  
        dodir "${WEEVELY}"/modules/net/external
        dodir "${WEEVELY}"/modules/generate
	dodir "${WEEVELY}"/modules/audit  
	dodir "${WEEVELY}"/modules/audit/external
	dodir "${WEEVELY}"/modules/backdoor  
	dodir "${WEEVELY}"/modules/bruteforce
		
	insinto "${WEEVELY}"/core
	doins core/*
	insinto "${WEEVELY}"/core/http
	doins core/http/*
	insinto "${WEEVELY}"/modules
	doins modules/*
	insinto "${WEEVELY}"/modules/file 
	doins modules/file/*
	insinto "${WEEVELY}"/modules/system 
	doins modules/system/*
	insinto "${WEEVELY}"/modules/find 
	doins modules/find/*
	insinto "${WEEVELY}"/modules/shell  
	doins modules/shell/*
	insinto "${WEEVELY}"/modules/sql 
	doins modules/sql/*
        insinto "${WEEVELY}"/modules/generate  
        doins modules/generate/*
	insinto "${WEEVELY}"/modules/audit  
	doins modules/audit/*
	insinto "${WEEVELY}"/modules/audit/external
	doins modules/audit/external/*
        insinto "${WEEVELY}"/modules/net  
        doins modules/net/*
        insinto "${WEEVELY}"/modules/net/external
        doins modules/net/external/*
	insinto "${WEEVELY}"/modules/backdoor  
	doins modules/backdoor/*
	insinto "${WEEVELY}"/modules/bruteforce
	doins modules/bruteforce/*
	dodir /usr/bin
	insinto /usr/bin
	echo -e "#!/bin/sh \n\ncd ${WEEVELY}\n" > weevely
	echo -e "./${PN}.py \$@\n" >> weevely
